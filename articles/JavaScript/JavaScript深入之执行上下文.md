...
用上面两段代码举例，checkscope 即将执行前的分析阶段，变量对象均为：
...
根据上节的内容，函数执行前会建立当前的变量对象（活动对象），然后把活动对象放在 [[scope]] 前端，形成完整的作用域链。
...
其中第 2 段代码的第 5 步非常关键，在[《JavaScript 深入之闭包》](./JavaScript深入之闭包.md)中详细讲述。
...
1. 函数中变量查找是查找作用域链，和当前的执行栈没关系。
2. 而作用域链由函数执行前的“准备工作”生成的活动对象，和函数创建时的内部属性 [[scope]] 两者共同构成。
3. [[scope]] 由什么构成呢？是函数创建时执行栈中的变量对象（们），它们为什么会存在呢？因为他们当时还存在于执行栈，之前入栈时经历过活动对象初始化。

因此，函数中的变量查找和当前的执行栈无关，但是和函数创建时的执行栈密切相关。